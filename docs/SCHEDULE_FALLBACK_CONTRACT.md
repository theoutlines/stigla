...
- Внутри группы: сначала live-строки, затем выжившие **Expected**-строки, затем
  **одна** свёрнутая Scheduled-ячейка — последней.
- Порядок групп между собой — по ближайшему событию группы (live/expected/
  scheduled), как список сейчас сортируется по времени. Сортировка «By comfort»
  и per-line фильтр — без изменений.
...
### Область
Только **шторка остановки**. Списки nearby, мини-карта и Planned-строки при
полном отсутствии live (поведение Фазы 1) — **вне** этой правки. Если у nearby
тот же симптом дублей — решается отдельно.
