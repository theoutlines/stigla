...
| `analytics_sweep` | worker runs the citywide "sentinel sweep": slow Cron rotation over ~160 mid-route stops through the existing SWR/arrivals path, so history covers every line — not just the stops users open (backend) | ON | ON | 2026-07-20 | permanent (gate + killswitch + auto circuit-breaker) — enabled in prod 2026-07-20 at the start tempo (day 20s / night paused; target 11s, see `config:sweep_interval_*`) |
