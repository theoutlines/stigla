...
## Осталось (по команде владельца)
1. **Merge `fix/flags-inline` → main** + деплой (backend `npm run deploy`, web
   build + `wrangler pages deploy --branch=main`, sha-верификация).
2. **Удалить 6 KV-ключей** (флаги больше не читаются — иначе «мёртвые» ключи):
   `flag:symbol_layer`, `flag:timed_trajectory`, `flag:live_position_only`,
   `flag:vehicle_direction_shape`, `flag:schedule_fallback`, `flag:schedule_map`.
   Только **прод-KV** (`9908…288`) — в нём они `=1`; **staging-KV пуст**, там
   удалять нечего. Удаление не обязательно для корректности (несуществующий флаг
   просто игнорируется), но убирает мусор.
