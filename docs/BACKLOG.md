...
- 🚧 Экран «Рядом» — nearby-список в духе Transit (2026-07-12,
  ...
  дедуп до ближайшей остановки, сортировка по ETA, автообновление 30 с. Новый
  бэкенд-эндпоинт `/api/v1/arrivals/nearby` (тот же ограниченный fan-out и 30-с
  кеш, что и `/vehicles/nearby`). Отложено (см. отчёт): learning-сортировка по
  частоте, пины избранного, Fleet-бейджи. Открытый вопрос данных: направление
  берётся из `all_stations` прибытия, а он у части ТС отсутствует → строки
  «→ None». Отчёт: `docs/reports/2026-07-12-nearby-list.md`.
