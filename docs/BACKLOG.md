...
- 💡 **Покрытие на основной карте**: при зум-ауте вместо кластеров остановок
  показывать слой свечения покрытия (порог по зуму + crossfade, lazy load
  GeoJSON) — плавный переход «обзор города → детальные остановки».
