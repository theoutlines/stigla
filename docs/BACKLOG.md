...
- 🚧 Экран «Рядом» — nearby-список в духе Transit (2026-07-12,
  `feature/nearby-list`, флаг `nearby_list`=OFF прод / ON staging). Draggable
  bottom sheet над картой: линии вокруг геопозиции, группировка линия+направление,
  дедуп до ближайшей остановки, автообновление 30 с. Новый бэкенд-эндпоинт
  `/api/v1/arrivals/nearby` (ограниченный fan-out ≤8 остановок + грид-индекс для
  резолва направления, чтобы не упираться в CPU-лимит воркера). Сортировка — два
  режима за флагом `nearby_sort_board`: по ETA (дефолт) или по времени до посадки
  (walk + ожидание; ON на staging). Отложено (см. отчёт): learning-сортировка,
  реальный headway в оценке посадки, пины избранного, Fleet-бейджи. Открытый
  вопрос данных: направление из `all_stations` прибытия, у части ТС пусто →
  строки «→ None». Отчёт: `docs/reports/2026-07-12-nearby-list.md`.
...
## Следующее — обновление GTFS-фида (`feature/gtfs-lifecycle`), приоритет поднят

- ⏭️ **Пересборка + регулярное обновление GTFS-фида.** Аудит линий
  (`fix/line-data-audit`, 2026-07-13) доказал, что это **не гигиена «на потом», а
  корень реального бага выдачи**: наш справочник stop→routes (скачан вручную
  ~8 мес. назад) системно отстал — на ~21% активных остановок живой поток несёт
  реальную линию, которой у нас нет, с сильным перекосом в пригород
  (400/500/800/900-серии: 491, 860x, 975, 953…). Источник/метод при этом
  корректны (H0/H1/H3 опровергнуты) — чинить нужно именно фид, а не upstream.
  Скачать свежий официальный GTFS → `npm run gtfs:build` → сверить масштаб
  изменений stop→routes → задеплоить; наладить регулярное обновление, чтобы не
  отставать снова. Промпт: `docs/prompts/prompt_gtfs_feed_lifecycle.md`.
  Обоснование и данные сверки: `docs/reports/2026-07-13-line-data-audit.md`.
- 💡 **App-side гашение устаревших линий (за флагом), отдельной задачей**
  (`prompt_stale_line_suppression`). Следствие того же аудита: экран остановки
  показывает состав как GTFS ∪ live, поэтому снятые/переименованные GTFS-«призраки»
  висят даже когда сети/эталонов их уже нет. Идея: при свежих живых данных строить
  чипы с приоритетом live-набора, GTFS-линии без подтверждения потоком/недавней
  историей гасить. Лечит «лишние линии» ещё до обновления фида. За флагом.
  Ссылка: `docs/reports/2026-07-13-line-data-audit.md`.
