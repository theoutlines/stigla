...
- ✅ **Фикс «стоячих меток» — за флагом `live_position_only`** (2026-07-13). На
  карту выводим только ТС с реальной live-позицией; плейсхолдеры источника
  (junk-гараж `P1..P999` / GPS на координате остановки) остаются в списке
  прибытий, но без метки. Обе поверхности + локализованный хинт пустой
  мини-карты. OFF на проде, ON на staging. Отчёт:
  `docs/reports/2026-07-13-gtfs-freshness.md`.
- ⏭️ **Фаза 2 — гашение GTFS-призраков (за флагом).** Экран остановки показывает
  состав как GTFS ∪ live, поэтому снятые/переименованные GTFS-«призраки» висят,
  даже когда сети/эталонов их уже нет. Гасить при приоритете live-набора. **[STOP
  & ASK] на критерий призрака** (ловушка «ночью не едет ≠ снята из сети»). За
  флагом. Детали — в промпте, раздел «Фаза 2».
- ⏭️ **Фаза 3 — добавление пригородного GTFS-датасета (охват).** Пригородные
  линии (700-е и пр.) идут **транзитом через город** и обслуживают городские
  остановки — для пассажира это обычные линии, а сейчас их нет в справочнике.
  Слить город+пригород в `build-gtfs.mjs` (stop_id общий, но обязательна
  валидация route_id-коллизий). Одобрено, по **[STOP & ASK]** до вливания
  (живые данные по 700-м? масштаб diff?). Делать после Фаз 1-2. Детали — в
  промпте, раздел «Фаза 3».
