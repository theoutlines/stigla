...
4. Если задача добавляет или меняет видимые экраны/состояния UI — обновить
...
5. Если задача добавляет или удаляет фиче-флаг — обновить реестр
   `docs/feature-flags.md` (строка на флаг: что гейтит / прод / staging / дата /
   статус; при удалении флага — убрать строку и KV-ключи). Реестр не должен
   протухать, как и галерея. Задачи, не трогающие флаги, реестр не трогают.
6. Отчёт обязан заканчиваться разделом «Как проверить» для владельца, не
...
7. Merge в main — только по явной команде владельца.
